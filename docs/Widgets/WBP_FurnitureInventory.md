...
**Phiên bản:** 3.35 | **Cập nhật:** 25/09/2026 09:40 — Event Construct **✓K2 export 25/09**: Sequence Then 2 (`GetAllActorsOfClass(BP_UndoManager)[0]` → SET `UndoManagerRef` → Bind `OnRestoreCompleted`) + Then 6 (tạo Inspector + 3 bind + Bind `UndoManagerRef.OnHistoryChanged → Handle_HistoryChanged → RefreshParamPanel`) — khớp doc.
**Phiên bản:** 3.34 | **Cập nhật:** 25/09/2026 09:30 — sửa dòng Keyboard Shortcuts: lưu / mở cảnh = phím **M** (menu Save/Load của project), KHÔNG có Ctrl+S/O (PIE cuhoang 25/09).
**Phiên bản:** 3.33 | **Cập nhật:** 24/09/2026 16:10 — **U2.4 + U2.5 (Undo Architecture, Interactive Edit Session) — PASS.** +2 Custom Event `Handle_ScalarBegin`/`Handle_ColorBegin` (bind `OnEditBegin` → `BeginInteractiveEdit`) · `Handle_ScalarCommit`/`Handle_ColorCommit` đổi `CaptureSnapshot(...)` → `CommitInteractiveEdit()` · +4 Cancel seam (`OnMeshSelected` nhánh Material, `CloseMaterialInspector`, `NotifyViewportSlotClick`, `OnSlotSwatchClicked`) · seam #6 mới: Event Construct bind `UndoManagerRef.OnHistoryChanged → Handle_HistoryChanged → RefreshParamPanel` · `RefreshParamPanel` seed đọc qua `GetSlotScalarParam`/`GetSlotVectorParam` (C++) thay cụm `Cast MI→MID + fallback hằng số` — hết hiện 0/FFFFFF cho slot chưa chỉnh. **Mức bằng chứng:** cuhoang build theo hướng dẫn, compile xanh + PIE PASS — CHƯA soi K2 export. Preview handler KHÔNG thêm guard `Sess_Active` (D-11). Xem `BP_UndoManager.md` v1.21, `DEVIATIONS.md` D-5..D-12.
...
| **— S7G7T3 Material Inspector (18/09/2026) —** | | |
| `MaterialInspectorRef` | WBP_MaterialInspector | SET ở Event Construct (`Create Widget` → `AddToViewport`); clear ở Event Destruct (R4). |
| `DT_ParamMap` | DataTable (Object Ref) | `DT_MaterialParamMap` (schema xem `Data/MaterialSlotService_Reference.md` mục UMaterialParamMap). Đọc trong `RefreshParamPanel` (`GetControlsForMaterial`) — **chưa xác nhận cơ chế gán giá trị** (default pin trên Designer hay SET runtime) qua K2Node, ghi chú lại để verify nếu chạm lại. |
| `bInspectorOpen` | Boolean, default False | SET trong `OpenMaterialInspector()`/`CloseMaterialInspector()`. |
| `bInspectorSuppressed` | Boolean, default False | SET đầu `SwitchInventoryMode` (`= NewMode != Material`). |
...
▶→ Branch( AND(CurrentInventoryMode==Material, IsValid(TargetFurnitureActor),
     ...
     True  → Get Player Controller(0) → PC
           ...
           ▶→ Branch( AND(bHit, OutActor==TargetFurnitureActor) )
                False → (dead-end)
                True  → UndoManagerRef.CancelInteractiveEdit()       ← v3.33 (U2.4) Cancel seam: đổi slot → hủy session TRƯỚC khi SET slot mới
                       ▶→ SET SelectedSlotIndex = OutSlotIndex
...
## S7G7T3 — Material Inspector Integration (18/09/2026, **T3.3 + T3.4 PASS**)
> S7G7T3 gồm T3.1 (`WBP_MaterialParamPanel`) + T3.2 (`WBP_MaterialInspector`) — cả 2 PASS, xem
> `Widgets/WBP_MaterialParamPanel.md`, `Widgets/WBP_MaterialInspector.md`. T3.3 (`RefreshParamPanel`)
> + T3.4 (lifecycle + 5 seam) — **PASS 18/09/2026**, nguồn
...
### `RefreshParamPanel()` — Function — AS-BUILT ĐẦY ĐỦ 18/09/2026 (K2Node export thật)
> Thay hoàn toàn trạng thái "đang xây" ghi ngày 17/09 — hàm đã build xong toàn bộ, test PASS
> (render/empty-state/bounds). 4 delegate bind ĐÃ NỐI nhưng 2 handler thân RỖNG (`Handle_ScalarPreview`/
> `Handle_ScalarCommit`, tương tự phía Color) — chờ thân thật ở T4, KHÔNG tính là gap của T3.3.

```
Function RefreshParamPanel()
▶→ Branch( IsInspectorVisible() )
     False → Return (không build gì)
     True  ▶→ Branch( IsValid(TargetFurnitureActor) )
              False → (xử lý như chưa chọn slot — panel trống)
              True  ▶→ GET TargetFurnitureActor.FurnitureMesh     ← KHÔNG Cast: biến
                                   ...
                                   True  ▶→ GetControlsForMaterial(SlotMaterial, DT_ParamMap) ●→ Controls
                                          ...
                                          ▶→ ForEach Controls (Control):
                                               Switch on Control.ControlType:
                                                 Scalar → GetSlotScalarParam(Mesh, SelectedSlotName, SelectedSlotIndex,
                                                              Control.ParamName) ●→ OutValue, ReturnValue      ← v3.33 (U2.5)
                                                    Select Float(ReturnValue ? OutValue : Control.MinValue) ●→ SeedF
                                                    ▶→ Create WBP_ParamScalarRow → Setup(...) →
                                                       Bind OnEditBegin→Handle_ScalarBegin,          ← v3.33 (U2.4)
                                                       Bind OnPreviewChanged→Handle_ScalarPreview,
                                                       Bind OnEditCommitted→Handle_ScalarCommit
                                                    ▶→ MaterialInspectorRef.AddParamRow(Row)
                                                 Color  → GetSlotVectorParam(Mesh, SelectedSlotName, SelectedSlotIndex,
                                                              Control.ParamName) ●→ OutValue, ReturnValue      ← v3.33 (U2.5)
                                                    Select LinearColor(ReturnValue ? OutValue : (1,1,1,1)) ●→ SeedV
                                                    ▶→ Create WBP_ParamColorRow → Setup(...) →
                                                       Bind OnEditBegin→Handle_ColorBegin,           ← v3.33 (U2.4)
...
> **[v3.33 — U2.5, 24/09] Seed đổi nguồn:** cụm `Cast MI→MID (bSuccess) → Get…ParameterValue / fallback hằng số`
> (18/09) ĐÃ THAY bằng `GetSlotScalarParam`/`GetSlotVectorParam` (C++, có exec pin — nối trên dây ▶→ trước `Create …Row`).
> Lý do: slot chưa chỉnh chỉ có MI, chưa có MID → Cast fail → panel hiện 0 / FFFFFF sai. BP không đọc được param trên MI
> (xác nhận 18/09 bên dưới), C++ `UMaterialInterface::Get…ParameterValue` đọc được mọi loại. Panel và Before của Undo
> giờ đọc CÙNG 1 nguồn. Fallback hằng số chỉ còn cho ca param không tồn tại. Mục "hands-on 18/09" dưới giữ làm lịch sử.
...
- `Get Scalar/Vector Parameter Value` KHÔNG nhận `Material Interface` làm Target — chỉ `Material
  Instance Dynamic` mới có 2 getter này. Pattern: `Cast MI → Material Instance Dynamic (bSuccess)`
  → True: gọi getter trên kết quả Cast ; False (slot chưa từng chỉnh, chưa có MID theo Đ4):
  fallback Scalar=`Control.MinValue`, Color=trắng `(1,1,1,1)`.
- Cả nhánh `Switch on EMaterialParamControl` — **Scalar VÀ Color** — đã build đủ theo pattern
  Cast+fallback trên. 4 delegate bind (Scalar×2, Color×2) ĐÃ NỐI, thân handler RỖNG chờ T4.
- Pin `Completed` của `ForEach Controls` nối `Return` — xác nhận đúng.

### `UpdateInspectorVisibility()` — Function MỚI, PASS (18/09/2026)
```
Function UpdateInspectorVisibility()
▶→ Branch( IsValid(MaterialInspectorRef) )
     True  ▶→ MaterialInspectorRef.SetVisibility( IsInspectorVisible() ? Visible : Collapsed )
     False → (dead-end)
```

### Event Construct — APPEND (18/09/2026) — ✓K2 25/09/2026 (Sequence Then 6)
```
▶→ Create Widget(WBP_MaterialInspector) ●→ SET MaterialInspectorRef
▶→ MaterialInspectorRef.SetVisibility(Collapsed)
▶→ AddToViewport
▶→ Bind MaterialInspectorRef.OnCloseRequested       → Handle_InspectorCloseRequested
▶→ Bind MaterialInspectorRef.OnResetParamsRequested → Handle_ResetParamsRequested
▶→ Bind MaterialInspectorRef.OnResetSlotRequested   → Handle_ResetSlotRequested
▶→ Bind UndoManagerRef.OnHistoryChanged             → Handle_HistoryChanged     ← v3.33 (U2.5, seam #6 mới)
```
> ⚠ `UndoManagerRef` phải được SET TRƯỚC node Bind này — bind vào None thì im lặng không bao giờ bắn. PIE U2.5 PASS
> (slider tự về đúng giá trị sau Undo/Redo command) → thực tế đã đúng thứ tự.
> **✓K2 25/09:** `UndoManagerRef` SET ở Sequence **Then 2** (`GetAllActorsOfClass(BP_UndoManager)[0]` → SET → Bind `OnRestoreCompleted`), khối trên ở **Then 6** → thứ tự đúng. Bind `OnHistoryChanged` KHÔNG có `IsValid(UndoManagerRef)` trước (L1) — UndoManager chưa sinh lúc Construct thì bind vào None, im lặng. Ghi nhận, chưa sửa.

```
Handle_HistoryChanged()   ← Custom Event (v3.33)
▶→ RefreshParamPanel()    ← tự guard IsInspectorVisible() ở đầu, gọi thừa vô hại
```
> Không có seam này: Undo command đổi mesh về giá trị cũ nhưng slider vẫn đứng giá trị mới (review #1 của task card).
> Tác dụng phụ có lợi: rebuild panel hủy row đang bị giữ chuột → chống ghi mồ côi khi Undo giữa lúc kéo (U6b).
Test PASS.

### Event Destruct — APPEND (18/09/2026, R4 chống VRAM leak)
```
▶→ Branch( IsValid(MaterialInspectorRef) )
     True  ▶→ MaterialInspectorRef.RemoveFromParent() ▶→ SET MaterialInspectorRef = None
     False → (dead-end)
```
Test PASS.

### `OpenMaterialInspector()` / `CloseMaterialInspector()` — Function MỚI, PASS (18/09/2026)
```
Function OpenMaterialInspector()
▶→ SET bInspectorOpen = True
▶→ Set Background Color(BTN_MaterialEdit, ColorButtonChoose)
▶→ UpdateInspectorVisibility()
▶→ RefreshParamPanel()

Function CloseMaterialInspector()
▶→ UndoManagerRef.CancelInteractiveEdit()      ← v3.33 (U2.4) Cancel seam: đóng panel = hủy session đang mở
▶→ SET bInspectorOpen = False
▶→ Set Background Color(BTN_MaterialEdit, ColorButtonDefault)
▶→ UpdateInspectorVisibility()
```
> **Lưu ý as-built quan trọng:** dùng node **`Set Background Color`** trên `BTN_MaterialEdit`
> (KHÔNG phải `SetHighlight` — hàm đó KHÔNG tồn tại trên Button này; task card rev6 ghi
> "SetHighlight" là SAI, đính chính ở đây). `ColorButtonChoose`/`ColorButtonDefault` là biến màu
> có sẵn từ trước (không phải mới).

### `BTN_MaterialEdit.OnClicked` — SỬA 18/09/2026
```
▶→ Branch( bInspectorOpen )
     True  ▶→ CloseMaterialInspector()
     False ▶→ OpenMaterialInspector()
```
Test PASS.

### `Handle_InspectorCloseRequested()` — Function MỚI, PASS (18/09/2026)
```
▶→ Call CloseMaterialInspector()
```

### `Handle_ResetSlotRequested()` — Function MỚI, PASS (18/09/2026)
```
▶→ Branch( IsValid(TargetFurnitureActor) AND SelectedSlotIndex>=0 )
     True  ▶→ GET TargetFurnitureActor.FurnitureMesh     ← KHÔNG Cast (cùng lý do RefreshParamPanel)
             ▶→ ResetSlotToAssetDefault(FurnitureMesh, TargetFurnitureActor.MaterialSlots [ref],
                                          SelectedSlotName, SelectedSlotIndex)
             ▶→ Get All Actors Of Class(BP_UndoManager) → Get(0) → CaptureSnapshot("ResetSlot")
             ▶→ RefreshSlotSwatches()
             ▶→ RefreshParamPanel()
     False → (dead-end)
```
...
▶→ Branch( IsValid(TargetFurnitureActor) AND SelectedSlotIndex >= 0 )
     True  ▶→ GET FurnitureMesh
             ▶→ SetSlotScalarParam(...) ●→ ok                ← GIỮ: đảm bảo MID mang giá trị cuối trước khi Commit đọc After
             ▶→ Branch(ok): True ▶→ UndoManagerRef.CommitInteractiveEdit()     ← v3.33 (U2.5) thay CaptureSnapshot("Chỉnh "+...)
                            False → (dead-end)
     False → (dead-end)
...
▶→ Branch( IsValid(TargetFurnitureActor) AND SelectedSlotIndex >= 0 )
     True  ▶→ GET FurnitureMesh ▶→ SetSlotVectorParam(...) ●→ ok
             ▶→ Branch(ok): True ▶→ UndoManagerRef.CommitInteractiveEdit()     ← v3.33 (U2.5) thay CaptureSnapshot("Đổi màu "+...)
                            False → (dead-end)
     False → (dead-end)
```
...
### 2 handler Begin + đổi Commit (U2.4/U2.5, v3.33 — 24/09/2026, PIE PASS)
> Label entry giờ do `CommitInteractiveEdit` tự dựng ("Chỉnh "/"Đổi màu " + ParamName) — node `Conv_NameToString`
> + nối chuỗi cũ bên cạnh `CaptureSnapshot` đã dọn. Preview handler GIỮ NGUYÊN (không thêm guard `Sess_Active` — D-11).
```
Handle_ScalarBegin(ParamName : Name)          ← Custom Event MỚI, bind OnEditBegin của WBP_ParamScalarRow
▶→ Branch( IsValid(TargetFurnitureActor) AND SelectedSlotIndex >= 0 )
     True  ▶→ UndoManagerRef.BeginInteractiveEdit(TargetFurnitureActor.PersistentID, SelectedSlotName,
                                                  SelectedSlotIndex, ParamName, Scalar)
     False → (dead-end)

Handle_ColorBegin(ParamName : Name)           ← đối xứng, Type = Color
```
> Cần dispatcher `OnEditBegin` của 2 row mang `ParamName` (nâng ở row v1.2) — bản đầu 0 input → không bind được
> (UE5 chỉ cho chọn Custom Event khớp 100% signature, sai là biến mất khỏi danh sách, không báo lỗi).
> `Q8:` Custom Event | guard IsValid Target && idx>=0 | dead-end False | không latent | 6A: Cancel seam.
> **PIE:** SESS-01 (1 dòng BEGIN/lần kéo, Before đúng), SESS-03 (1 entry/lần thả), Color W3 (BEGIN có bắn khi chỉnh màu).
...
> seam #1-#5 đã build; #6 (`OnSceneRestored`)/#7 (`EndParamSession`) chưa build, xem cảnh báo đầu
> mục. rev6 cũng ghi seam #5 ở ĐẦU hàm `SwitchInventoryMode` — **SAI**, đã sửa đặt ở CUỐI nhánh
> Material (xem seam #5 dưới) — đóng dấu `[HISTORICAL]` phần liên quan trong rev6, không xóa.

**[v3.33 — U2.4] `OnSlotSwatchClicked` +Cancel seam ĐẦU hàm:** `UndoManagerRef.CancelInteractiveEdit()` là node đầu tiên,
node đầu cũ nối sau nó. (Thân đầy đủ của event này CHƯA có as-built trong doc — chỉ biết SET SelectedSlotIndex + loop
highlight theo Array Index; chèn "đầu hàm" không phụ thuộc cấu trúc giữa.)

**#1 — `OnSlotSwatchClicked`, APPEND cuối hàm:**
```
▶→ BTN_MaterialEdit.SetIsEnabled(True)
▶→ RefreshParamPanel()
```

**#2 — `NotifyViewportSlotClick`, APPEND sau `Call HighlightSwatchByIndex(OutSlotIndex)`:**
```
▶→ BTN_MaterialEdit.SetIsEnabled(True)
▶→ RefreshParamPanel()
```

**#3 — `OnMeshSelected` nhánh Material, APPEND cả 2 nhánh True/False:**
```
▶→ BTN_MaterialEdit.SetIsEnabled(False)
▶→ RefreshParamPanel()
```
> Lệch so với rev6: rev6 ghi gọi `EndParamSession()` ở seam này (#6/#7 của rev6) — **ĐÃ BỎ**, hàm
> đó thuộc T4b, chưa tồn tại. Không gọi gì tương đương ở đây trong T3.4.

**#4 — `LoadAndApplyMaterial`, APPEND vào nhánh `Completed` (sau `AddRecentMaterial`):**
```
▶→ RefreshParamPanel()
```

**#5 — `SwitchInventoryMode`:**
```
ĐẦU hàm:
▶→ SET bInspectorSuppressed = (NewMode != Material)
▶→ UpdateInspectorVisibility()
...
> khi swatch đã dựng xong — test PASS. rev6 phần mô tả vị trí cũ đóng dấu `[HISTORICAL]`, giữ để
> tra lý do đã từng nghĩ sai.
...
**Nhánh MATERIAL (v1.1 + v2.4 guard + T4 Equal-guard 18/09 + U2.4 Cancel seam 24/09):**
```
Branch CurrentInventoryMode == Material:
  T →
    UndoManagerRef.CancelInteractiveEdit()                  ← v3.33 (U2.4): TRƯỚC khi đổi Target (cả chọn mới lẫn deselect).
                                                               KHÔNG đặt ở nhánh Replace. Không session → tự Return, vô hại.
...
— việc chèn `ResolveActiveComboForSave()` vào `CB_SaveCombo_Handler` đã phân phối vào
`BP_FurnitureInputManager.md` v3.14 (✓K2 25/09/2026).
```
...
## Keyboard Shortcuts
Q/W/E/R = Select/Move/Rotate/Scale | Delete = xóa | Ctrl+Z / Ctrl+Shift+Z = Undo/Redo (sửa 24/09 — trước ghi nhầm Alt+Z) | M = menu Save/Load của project (sửa 25/09 — trước ghi nhầm Ctrl+S/O) | I = Inventory | Esc = Deselect | Ctrl+G / Ctrl+Shift+G = Group/Ungroup (Sprint 3)
...
| 3.35 | 25/09/2026 09:40 | Event Construct ✓K2 (Then 2 SET UndoManagerRef + Bind OnRestoreCompleted; Then 6 Inspector + Bind OnHistoryChanged → Handle_HistoryChanged → RefreshParamPanel). Khớp doc. Ghi nhận thiếu IsValid trước Bind (L1). |
...
- [[BP_UndoManager]] — nghe lịch sử đổi → refresh panel · Bind OnHistoryChanged → RefreshParamPanel() ✓K2
...
- [[BP_UndoManager]] — báo lịch sử vừa đổi (cuối AppendEntry) · Broadcast OnHistoryChanged ✓K2
- [[BP_UndoManager]] — báo lịch sử vừa đổi (cuối nhánh Undo / Redo) · Broadcast OnHistoryChanged (Undo/Redo)
