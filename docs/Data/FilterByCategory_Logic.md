...
Branch ActiveSpecialCategory == "Recent":
  ...
  F → (chưa chọn → select)
      Branch ActiveSpecialCategory == "":
        T → (không có special nào active — không cần làm gì thêm)
...
BTN_Tab_Material OnClicked:
  ...
  → Branch ActiveSpecialCategory != "":
      ...
      F → (không làm gì)
```
> KHÔNG sửa bên trong SwitchInventoryMode — xử lý ở Tab button OnClicked sau khi SwitchInventoryMode hoàn tất → CurrentInventoryMode đã được SET đúng.
...
- **"Recent" và "Favorite" KHÔNG gọi FilterBySearch** — tránh CategoryFilter class variable pollution
- **ForEach False branch để trống** — Blueprint tự tiếp tục iteration
- **Add Item / Set List Items — Item pin phải được nối vào DA** — nếu không nối, Add Item chạy nhưng không add gì
- **String to Name( Get Object Name(DA) ) == RowName** — so sánh Name với Name, không dùng Name to String
- **CategoryFilter parameter vs class variable** — trong FilterByCategory, LUÔN dùng wire từ input pin, không GET node riêng (GET đọc class variable, không phải parameter)
