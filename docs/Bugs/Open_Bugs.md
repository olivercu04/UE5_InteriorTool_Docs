...
| Bug-UndoAcrossLoad | [OPEN, 25/09] Sổ Undo KHÔNG xoá khi Load cảnh (menu Save/Load) → Ctrl+Z ngay sau Load đưa về cảnh TRƯỚC Load (PIE cuhoang 25/09). Người dùng dễ "undo mất" cả bản vừa mở; các app thiết kế thường xoá sổ + ghi mốc mới khi mở file | 🟡 Trung bình | Chưa sửa — cần quyết: xoá sổ + `CaptureSnapshot("Initial")` sau Load (đề xuất). Chỗ móc: sau khi mọi actor `ActorLoaded` xong — chưa có điểm "Load xong" rõ ràng. Xem Architecture_Map 5t |
| Bug-BoxSelectCtrl-MultiSnapshot | ✅ FIXED 25/09 — Ctrl + quét khung trúng N món → ghi N mốc "BoxSelect" (PIE: 3 món = Ctrl+Z 3 lần). Root cause ✓K2 25/09: `FinishBoxSelect` nhánh Ctrl nối `ToggleActor.then` → `CaptureSnapshot` BÊN TRONG Loop Body | 🟡 Trung bình | Fix: nối `ForEachLoop.Completed` → `CaptureSnapshot`, bỏ dây từ `ToggleActor.then`. PIE PASS 3/3 (Ctrl+Z 1 lần, Redo 1 lần, quét không Ctrl regression) |
