...
| Bug-UndoAcrossLoad | ✅ FIXED 25/09 — Sổ Undo không xoá khi Load → Ctrl+Z sau Load về cảnh trước Load | 🟡 Trung bình | Fix: `BP_FurnitureSceneManager.OnLoadButtonClicked` → Wait For Operation(Load) → `BP_UndoManager.ResetHistoryToBaseline("Load")` (Clear sổ → CurrentIndex=-1 → CaptureSnapshot). PIE PASS: `LOAD DONE n=3`, Ctrl+Z sau Load đứng yên, Move→Undo→Undo đúng |
