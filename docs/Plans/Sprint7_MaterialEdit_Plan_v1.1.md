...
| Đ5 | `[HISTORICAL — OVERRIDE 15/09/2026]` UMG không có color picker runtime → v1 = bảng màu preset + 3 slider; bánh xe màu = backlog. **G7.0a (14/09) chứng minh ngược lại:** 3 primitive Slate (`SColorWheel`+`SSimpleGradient`+`SSlider`) sống packaged Shipping → bánh xe màu LÀ control màu chính của G7 (`InteriorColorPicker`), preset swatch tụt xuống đường phụ tùy chọn. Xem `Widgets/InteriorColorPicker.md`, `Sprints/Sprint7/15-09-2026_S7G7_T1-T5_ExecutionPlan.md` mục 0(a). |
