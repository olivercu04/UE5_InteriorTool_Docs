...
## [DOC-DEBT — ĐÃ ĐÓNG 25/09/2026, K2] BP_FurnitureInputManager.md nợ cập nhật ResolveActiveComboForSave trong CB_SaveCombo_Handler — 07/08/2026
...
| 21/09/2026 (U1 — lệch scope, phát hiện lúc PIE test) | **Q10 producer table thiếu 1 đường.** `21-09-2026_U1_PersistentIdentity_TaskCard.md` §Q10 gốc ghi `SpawnFurnitureCopy (card-drag, paste, combo spawn)` — gộp nhầm card-drag chung với SpawnFurnitureCopy. Thực tế (đối chiếu `AI_Implementation_Rules.md` + `Widgets/WBP_DragOverlay_FurnitureCard.md`): card-drag spawn actor riêng qua `PreviewActorRef` (`On Drag Detected` spawn, `On Drop` finalize + ADD tag "FurnitureSpawned"), KHÔNG đụng `SpawnFurnitureCopy` bao giờ. Nếu không vá, actor spawn từ card-drag (đường đặt đồ phổ biến nhất) sẽ có tag `FurnitureSpawned` nhưng `PersistentID` rỗng vĩnh viễn — vi phạm invariant chính của U1. Phát hiện khi cuhoang test PIE U1.2 (kéo card vào scene, Print String không ra gì). cuhoang duyệt thêm producer thứ 4 (`On Drop`, ensure ngay sau `Cast PreviewActorRef → BP_FurnitureActor`, trước `Set Static Mesh`). Task card Q10 đã sửa tại chỗ (không đánh version riêng — file PLAN, chưa as-built). Cần chèn node BP + test lại trước khi đóng U1.2. |
| 24/09/2026 (U2.3 — D-1) | **`BuildSceneSnapshotBase` cache InputManager qua local var.** Bản tách dùng Local var `FurnitureInputManagerLocalVar : BP_FurnitureInputManager` — SET 1 lần (`Get All Actors Of Class(BP_FurnitureInputManager) → Get(0)`) đầu hàm, dùng lại ×3 chỗ (Step 0b EditModeStack / Step 4 SelectedActors / Step 6 ActiveMode), thay 3 lần `Get All Actors Of Class → Get(0)` riêng lẻ của `CaptureSnapshot` gốc. Chủ đích (P2 — không query lặp), hành vi tương đương. Ceiling: chấp nhận. Trigger: không. Xem `BP_UndoManager.md` v1.20 §BuildSceneSnapshotBase. |
| 24/09/2026 (U2.3 — D-2) | **[DOC-DRIFT] Step 3 THẬT có 2 tầng cast.** `CaptureSnapshot`/`BuildSceneSnapshotBase` Step 3: `Cast To StaticMeshActor(Array Element) → Cast To BP_FurnitureActor(AsStaticMeshActor)` — 2 tầng. Canonical cũ (`BP_UndoManager.md` ≤v1.19) ghi 1 tầng (`Cast To BP_FurnitureActor` thẳng) — doc SAI từ trước, sửa theo thật (K2 export xác nhận). Không đổi node flow thật, chỉ sửa mô tả. |
| 24/09/2026 (U2.3 — D-3) | **[DOC-GAP] `RestoreSnapshot` có input thứ 2 `PreviousActor`.** Chữ ký thật = `RestoreSnapshot(IndexHistory : Integer, PreviousActor : BP_FurnitureActor)` — canonical trước chỉ ghi 1 param `IndexHistory`. `Undo`/`Redo` nhánh Snapshot truyền `PreviousActor = GET InputManager.SelectedFurnitureActor`. Bổ sung chữ ký ở `BP_UndoManager.md` v1.20; điểm tiêu thụ chính xác trong THÂN hàm CHƯA soi K2 export riêng (thân giữ nguyên — §13: không ground truth thì báo, không sửa). Mâu thuẫn cùng gốc: `Data/Data_Structures.md` "BP_UndoManager Functions" vẫn ghi 1 param — CHƯA sửa (ngoài phạm vi delta U2.3, chờ cuhoang quyết). |
| 24/09/2026 (U2.3 — D-4) | **`ApplyParamCommand` xây sớm ở U2.3 (ngoài scope, đã review).** cuhoang xây sẵn thân `ApplyParamCommand` (§5.6) ngay U2.3 thay canary Print (Sonnet đề xuất, chấp nhận) — dây sẵn vào nhánh True của Undo/Redo nhưng KHÔNG bao giờ chạy trong U2.3 (chưa có command entry). ✅ **Nợ review đã GỠ:** K2 export thân (`ApplyParamCommand`) đã đối chiếu §5.6 task card trong phiên — đúng (Resolve→bFound guard→GET Mesh→chọn Before/After theo `bUseBefore`→SetSlot Scalar/Color→Return). Không cần review lại đầu U2.4. |
| 24/09/2026 (U2.4 — D-5) | **Thứ tự BUILD lệch task card: `CommitInteractiveEdit` (card xếp U2.5) build trong U2.4.** `BeginInteractiveEdit` §5.2 gọi `CommitInteractiveEdit()` (settle session cũ) → Blueprint không compile nếu hàm chưa tồn tại. Thứ tự thật: session vars → `ApplyParamCommand` → `Commit` → `Begin` → `Cancel` → UI. KHÔNG đổi phạm vi PIE: U2.4 vẫn chỉ test SESS-01/05/07; UI Commit (`Handle_*Commit`) vẫn nối ở U2.5. Bài học: task card chia gate theo nhịp PIE nhưng không kiểm phụ thuộc compile giữa các hàm. |
| 24/09/2026 (U2.4 — D-6) | **Tên/biến card không đặc tả:** enum `Sess_Phase` card ghi "E (None | Previewing)" → đặt `E_ParamSessionPhase` (tách khỏi `E_HistoryEntryKind`: loại entry ≠ trạng thái session). + class var tạm `CommitEdit_Label:String` — `Commit` là Custom Event (không Local Var, L9), 2 nhánh Scalar/Color cần dồn label về 1 pin input (1 pin chỉ nhận 1 dây). Prefix theo quy ước biến tạm 1 event. |
| 24/09/2026 (U2.4 — D-7) | **[GAP card §5.4] `CommitInteractiveEdit` tự Resolve actor.** Card viết "đọc After hiện tại: GetSlot*Param(...)" không nói Mesh lấy từ đâu. UndoManager không có `TargetFurnitureActor` (của widget) → Resolve lại bằng `Sess_Cmd.EntityID` y hệt Begin; `bFound=False` → tắt session, không entry. Đúng QĐ3 (đọc từ core). |
| 24/09/2026 (U2.4 — D-8) | **[Q10] `OnEditBegin` của 2 row +`ParamName:Name`.** Card QĐ5 coi `OnEditBegin` "có sẵn, chỉ cần bind" — thật ra 0 input → `Bind OnEditBegin` không chọn được `Handle_*Begin(ParamName)` (sai signature, UE5 ẩn khỏi danh sách, không báo lỗi). Cùng loại lỗi producer-contract với `OnPreviewChanged`/`OnEditCommitted` T4. `OnEditBegin` chưa có consumer → an toàn. `WBP_ParamScalarRow`/`WBP_ParamColorRow` v1.2. |
| 24/09/2026 (U2.4 — D-9) | **Guard session ở Undo/Redo = gọi thẳng `CancelInteractiveEdit()` node đầu, KHÔNG bọc `Branch(Sess_Active)`** như card §5.7/5.8 vẽ — Cancel đã tự guard bên trong, bọc thêm là lặp. Hành vi y hệt. |
| 24/09/2026 (U2.5 — D-10) | **`Broadcast OnHistoryChanged` CHỈ ở nhánh Command của Undo/Redo** (card §5.1c/5.7/5.8/§6 ghi broadcast ở Capture/Commit/Undo/Redo/Jump). Nhánh Snapshot không broadcast: `RestoreSnapshot` async (destroy+respawn), refresh ngay sau đó chạy lúc actor chưa sẵn — `ApplyRestoredActor` đã lo. Broadcast ở AppendEntry/Commit/Jump + test HISTUI-03 = **U2.6**. Trigger mở rộng: U2.6. |
| 24/09/2026 (U2.5 — D-11) | **KHÔNG thêm guard `Sess_Active` vào 4 handler Preview/Commit** (card §5.3/§5.10 mâu thuẫn chỗ này). Nghi vấn "ghi mồ côi" (Cancel bằng phím lúc đang giữ slider → thả → handler vẫn SetSlotParam, không entry) — **test U6b BÁC BỎ**: Undo → `OnHistoryChanged` → `RefreshParamPanel` hủy row đang giữ → thao tác sau đó rơi vào widget đã hủy. Ceiling: bảo vệ DỰA VÀO rebuild panel. **Trigger buộc thêm guard:** xuất hiện đường Cancel KHÔNG rebuild panel (vd phím tắt đổi tab — watch-item T5 chưa test). Ngoài ra U6 ban đầu cũng không lộ vì Undo Move respawn → `SelectedSlotIndex=-1` chặn ở guard handler (bug che bug). |
| 24/09/2026 (U2.5 — D-12) | **[PLAN-SAI, card W2/QĐ3] Reader Before chỉ đọc MID là sai → đổi sang đọc `UMaterialInterface`.** Card chấp nhận "false→fallback Min/trắng" cho slot chưa có MID — thực tế đó là ca PHỔ BIẾN NHẤT (lần chỉnh đầu mỗi slot) → Before = 0/trắng → Undo về sai (PIE: Tint undo về FFFFFF). Fix 3 tầng: (1) C++ `GetSlotScalar/VectorParam` đọc MI gốc khi chưa có MID (`c23b585`); (2) `RefreshParamPanel` seed đọc qua 2 hàm này thay Cast MID + fallback hằng số (panel = Undo cùng 1 nguồn); (3) C++ `InteriorColorPicker.SetColor` lưu `InitialColor` trước — `Setup` gọi `SetColor` trước khi row được gắn nên bánh xe mất màu. Cả (2)(3) là lỗi CÓ TỪ T3/G7.0, bị che vì seed cũ luôn trắng. F1–F4, C1–C5 PASS. |
| 24/09/2026 (ngoài U2 — D-13) | **Đóng B-gizmo (treo 15/06) trong phiên U2.** `UpdateGizmo` nhánh `==1` +`DeactivateGizmo` trước `ActivateGizmo` (toggle + `RestoreSnapshot` gọi `SelectActors` 2 lần). Ngoài scope task card, làm vì cuhoang yêu cầu khi gặp lại ở test U5; 1 node, test G1–G6 PASS. `BP_FurnitureInputManager.md` v3.9. |
| 24/09/2026 (sửa ngày) | Các dòng U2.3 D-1..D-4 trước ghi nhầm "25-09-2026" → sửa thành **24/09/2026** theo xác nhận cuhoang. Áp đồng loạt cho `BP_UndoManager.md`, `01_Session_State.md`, `Learning_System.md`, `PROGRESS.md`. |
| 24/09/2026 16:40 (U2.6 — D-10 sửa) | **D-10 lập luận sai một nửa → broadcast đủ cả nhánh Snapshot.** `RestoreSnapshot` chạy tuần tự tới Step 7 (`OnRestoreCompleted`) rồi mới trả về — không phải "async chưa xong" như D-10 ghi. Thêm broadcast cuối nhánh Snapshot Undo/Redo + cuối `AppendEntry` → HISTUI-03 PASS. Giờ khớp task card §6 (Capture/Commit/Undo/Redo/Jump). |
| 24/09/2026 (U2 — plan-level, task card §10) | **3 lệch có chủ đích so với plan Undo Architecture, chốt từ task card U2, ghi as-built:** (1) **QĐ6** — Resolver `ResolveByPersistentId` có caller THẬT đầu tiên ở U2 (`ApplyParamCommand`/`Begin`/`Commit`), không phải U3 như plan `18-09-2026_UndoArchitecture_Foundation_v1.md` — undo command bắt buộc resolve theo ID (con trỏ cũ chết sau respawn). (2) **Hướng B (additive hybrid)** — command entry VẪN kèm full snapshot, trụ E (`{Before,After}` snapshot + gỡ ảnh thừa, RAM win) hoãn sang bước SUBTRACT sau U3. (3) **Đ12 texture** — không có command Texture (`EParamCmdType` chỉ Scalar/Color, `RefuseTexture()` luôn false, Spec `UNDO-CMD-TEX` xanh). |
| 24/09/2026 19:15 (doc-drift phím tắt) | **Undo/Redo = Ctrl+Z / Ctrl+Shift+Z, KHÔNG phải Alt+Z.** Nguồn đúng: `BP_FoffPlayerController.md` (IA_FurnitureUndo Started + check Shift; IA_FurnitureRedo chord Ctrl+Shift+Z) + cuhoang xác nhận. Đã sửa canonical: `BP_UndoManager.md` 2 heading, `WBP_FurnitureInventory.md` bảng phím tắt, `Architecture_Map` Phần 5. KHÔNG sửa file Plan/Planning/task card (tài liệu lịch sử — `Planning/07_Testing_Strategy.md`, `MultiSelect_Group_ComboMesh_Plan.md`, `UE5_InteriorTool_Overview.md`, task card U2 vẫn ghi Alt+Z). + **Doc gap** `BP_UndoManager` RestoreSnapshot Step 4 thiếu dòng `SET NewActor.MaterialSlots` so với changelog v1.17 — chờ K2 Step 4. |
| 25/09/2026 09:10 (doc-drift Input) | **Input Action nội thất nằm ở `BP_FurnitureInputManager`, không phải `BP_FoffPlayerController`** — từ Gate 1.5 B2 (18/08): IM tự `AddMappingContext(LM_FurnitureInput, P5)` ở BeginPlay (✓K2 25/09), giữ suốt phiên; mở/đóng inventory không đổi bộ phím. `BP_FoffPlayerController.md`, `Flows/Nudge_Flow.md`, `Flows/CopyPaste_Flow.md` chưa cập nhật từ đó → gắn banner LỖI THỜI (không xoá nội dung cũ). Bộ não v1.8 vẽ sai theo doc cũ (5c, 5g, 5k, 5m + cạnh `PC→…` Phần 3) → sửa ở Architecture_Map v1.11. Phát hiện: cuhoang. |
| 25/09/2026 10:30 (doc-drift Spawn Order) | **Các manager do `WBP_FOFF_ToolDemo` Event Construct Then 11 sinh, KHÔNG phải Level Blueprint** (✓K2 25/09). Lệch so với plan `Planning/Integration_Guide.md` Bước 10: IM sinh TRƯỚC GizmoController (plan: Gizmo trước), không Cast, `TransformerPawnRef` không SET; thêm `BP_GroupsContainer`, `BP_ComboManager`, `WBP_Toast` (ToastRef gán vào `BP_FurnitureSceneManager`). Sửa canonical `BP_FurnitureInputManager.md` v3.13 + Architecture_Map v1.17. KHÔNG sửa `Planning/Architecture_Overview.md`, `Planning/Integration_Guide.md` (tài liệu kế hoạch lịch sử). |
| 25/09/2026 10:40 (đóng DOC-DEBT 07/08) | `CB_SaveCombo_Handler` ✓K2 25/09: `ResolveActiveComboForSave()` có thật, đứng SAU `IsValid(FoundWidgets[0])` (plan 7b.1 ghi đầu nhánh True của guard ≥2 — lệch vị trí vô hại). 3 pin xuống `OpenSaveComboDialog`. Đóng mục `[DOC-DEBT] … ResolveActiveComboForSave trong CB_SaveCombo_Handler — 07/08/2026`. `BP_FurnitureInputManager.md` v3.14, Architecture_Map v1.18. |
