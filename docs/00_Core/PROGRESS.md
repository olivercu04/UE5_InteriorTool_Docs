...
| 08/09/2026 (resequence G4-G10) | **Sprint 7 nửa sau — resequence (Opus, PLAN, CHƯA giao Sonnet).** Bản đồ gate G4-G8 gốc (05/07, chưa rà lần nào) thay bằng G4-G10 mới: G4 xác nhận trục-A (chips/swatch đã có, không code mới) → G5 kéo-thả vật liệu (đường phối màu CHÍNH, rủi ro Cao — có [VERIFY] trace-on-drop) → G6 click-vào-mesh chọn slot (đường phụ) → G7 động cơ panel param + từ điển tạm → G8 điền từ điển thật + bộ test T7 → G9 pattern gạch + đóng Đ12 → G10 regression tổng + VRAM + docs. Lý do: G0d (kháo sát collision 27/08) + G1 as-built (service 14 hàm PASS) + quyết định UX 08/09 (kéo-thả là đường chính, click-vào-mesh là đường phụ) khiến 3/4 việc G4 gốc bị bỏ/dời backlog (re-import test, Replace giữ material trùng tên, multi-select khác loại theo tên). Plan gốc G4-G8 đóng dấu `[HISTORICAL]` trong `Plans/Sprint7_MaterialEdit_Plan_v1.1.md` v1.11, giữ nguyên nội dung để tra lý do. KHÔNG đụng Blueprints/Widgets (delta là plan, chưa as-built). Bar Sprint 7 KHÔNG đổi (chưa gate nào trong G4-G10 chạy). Chờ cuhoang duyệt plan trước khi giao Sonnet vào G4. Nguồn: `DELTA_Opus_S7_G4-G10_Resequence_08sep2026.md`. Chi tiết: `01_Session_State.md`, `MERGE_LOG.md`. |
| 08/09/2026 (S7.G4 ĐÓNG) | S7.G4 xác nhận trục-A — ĐÓNG. Round-trip material theo tên qua save/load PASS: apply 2 slot khác tên (Gối và bọc giường_Fabric, Chăn hè_Fabric) → Save → Load → RestoreMyMaterialSlots chạy ResetAllSlotsToAssetDefault rồi ApplyLoadedMaterialToSlot đúng cả 2, khớp 100% mốc trước Save. Không dùng re-import (lý do: ResolveSlotIndex resolve theo tên runtime, xem resequence 08/09). Sang G5 — kéo-thả vật liệu, mở màn bằng [VERIFY G5.0] trace-on-drop. Nguồn: phiên chat 08/09/2026. |
...
| 15/09/2026 (S7G7 T1-T5 plan) | Plugin `InteriorColorPicker` copy về `Lighting_Mnger` xong (build sạch, plugin enabled, PIE chạy được) — nợ từ G7.0a đóng. Opus giao execution plan **S7G7T1-T5** (Material Param Panel — id chuẩn hóa theo R-DOC-TASKID): T1 = `DT_MaterialParamMap`+`GetControlsForMaterial` (C++, task DUY NHẤT có code mới); T2 = `WBP_ParamScalarRow`/`WBP_ParamColorRow`; T3 = `WBP_MaterialParamPanel`; T4 = nối `UMaterialSlotService` (**integration gate**, rủi ro cao nhất còn lại — undo tái dùng `CaptureSnapshot`/`ApplyParamsJsonToSlot` có sẵn, 0 dòng code undo mới); T5 = multi-select áp param cả cụm. Đ5 (`Plans/Sprint7_MaterialEdit_Plan_v1.1.md`) đóng dấu `[HISTORICAL — OVERRIDE]` — bánh xe màu là control chính, không còn fallback. Plan CHƯA as-built. Nguồn: `Sprints/Sprint7/15-09-2026_S7G7_T1-T5_ExecutionPlan.md`. Xem `01_Session_State.md`, `Widgets/InteriorColorPicker.md`. |
| 15/09/2026 (S7G7T1 as-built) | **S7G7T1 ĐÓNG — PASS 4/4.** `GetControlsForMaterial`+`DT_MaterialParamMap` chạy đúng — class riêng `UMaterialParamMap` (`MaterialParamMap.h/.cpp` trong `FurnitureToolkit`), không nhét vào `MaterialSlotService`. Chốt so CON TRỎ base material (`GetBaseMaterial()` sau `LoadSynchronous`), KHÔNG path string — path FAIL với MID (MID có path khác MI gốc), con trỏ resolve đúng cả 2. Test 4/4: MI trong từ điển (2 row) · MI ngoài từ điển (0 row) · MID từ đúng MI (2 row, chứng minh không bị đánh lừa) · đọc field đúng. Nợ nhẹ không chặn: `LoadSynchronous` mỗi row trong loop (vô hại 2 dòng, cân nhắc cache ở G8). Backlog mới (SAU Sprint 7, cần duyệt đồng nghiệp): Static Switch runtime toggle — ưu tiên hướng Lerp+Scalar (sửa master, tái dùng `SetSlotScalarParam`) thay MI-swap. Chi tiết: `Data/MaterialSlotService_Reference.md` mục "UMaterialParamMap", `DEVIATIONS.md`. |
| 15/09/2026 | Fix `Bug-MaterialSlots-MissingInClipboard` (Copy/Paste/Duplicate mesh mất material). `SpawnFurnitureCopy` +input `MaterialSlots`. Test 6/6 PASS. Không thuộc Sprint 7 (bug fix ngoài phạm vi gate, xử lý ngay theo yêu cầu cuhoang). |
| 15/09/2026 (S7G7T2) | **S7G7T2 ĐÓNG.** `WBP_ParamScalarRow` + `WBP_ParamColorRow` PASS. +`HexToLinearColor` (C++, cùng class `UMaterialParamMap`). Preset swatch v1 bỏ, dồn Project Palette/Recent Colors (backlog UX sau G7). 2 bug fix trong phiên (SpinBox init thiếu wire, dead-end pin `OnPreviewChanged`). Chi tiết: `Widgets/WBP_ParamScalarRow.md`, `Widgets/WBP_ParamColorRow.md`, `01_Session_State.md`. |
| 17/09/2026 (S7G7T3 dở dang) | **S7G7T3 ĐANG DỞ — KHÔNG ĐÓNG.** T3.1 (`WBP_MaterialParamPanel`, tạo mới — cuhoang xác nhận chưa từng build trước đó) PASS + T3.2 (`WBP_MaterialInspector`, tạo mới) PASS 7/7. `IsInspectorVisible()` node-verified. T3.3 (`RefreshParamPanel`, tích hợp `WBP_FurnitureInventory`) **đang xây, chưa test đủ**: nhánh Scalar built-chưa-verify, nhánh Color chưa build, phần đầu hàm chưa as-built. KHÔNG tick S7G7T3 xong cho tới khi T3.3 PASS 5/5. Chi tiết: `Widgets/WBP_MaterialParamPanel.md`, `Widgets/WBP_MaterialInspector.md`, `Widgets/WBP_FurnitureInventory.md` mục "S7G7T3", `01_Session_State.md`. |
| 18/09/2026 (S7G7T3.3+T3.4 PASS) | **T3.3 + T3.4 PASS.** `RefreshParamPanel()` build đầy đủ (nhánh Scalar+Color, fallback MID, empty-state, bounds — test PASS render/empty-state/bounds). T3.4: lifecycle Construct/Destruct + `OpenMaterialInspector`/`CloseMaterialInspector` (dùng `Set Background Color`, KHÔNG phải `SetHighlight` như task card rev6 ghi sai) + `Handle_ResetSlotRequested` PASS + `Handle_ResetParamsRequested` stub (chờ T4) + 5 seam APPEND (seam #5 sửa vị trí — CUỐI nhánh Material, rev6 ghi ĐẦU hàm là sai). Vá lỗ hổng đường kéo-thả material (`BP_FurnitureActor.ApplyMaterialByRowName` X3, rev6 không liệt kê seam này) — xem `Blueprints/BP_FurnitureActor.md` v2.5. Bug mới `Bug-MaterialEdit-EnableState` [OPEN, 🟡] — nút Material Edit không disable đúng lúc trong 1 số đường vào Material mode, nghi seam #3. Còn treo (T4/T4b): seam #6/#7 (`OnSceneRestored`/`EndParamSession`), 4 delegate handler thật, `Handle_ResetParamsRequested` thật. Chi tiết: `Widgets/WBP_FurnitureInventory.md` v3.30 mục "S7G7T3", `01_Session_State.md`, `Bugs/Open_Bugs.md`. |
| 18/09/2026 (đóng bug + Q10) | **Đóng `Bug-MaterialEdit-EnableState`.** Root cause thật (Print A xác nhận) KHÁC nghi vấn seam #3: 2 lỗi "entry point cũ không rà lại khi seam mới ra đời" — `RefreshSlotSwatches` thiếu `HighlightSwatchByIndex`, `BTN_ResetSlot`/`BTN_ResetAll` thiếu `RefreshParamPanel`. Fix+test PASS cả 3 chỗ. Elevate thành rule chung `AI_Implementation_Rules.md` **Q10 — FLOW COVERAGE GATE (Cross-Flow Impact Audit)** v2.20, đặt TRƯỚC Q9. Chi tiết: `WBP_FurnitureInventory.md` v3.31, `Bugs/Open_Bugs.md`. |
| 18/09/2026 (**S7G7T4 GATE ĐÓNG**) | **T4 — INTEGRATION GATE ĐÓNG, PASS.** 5 handler thật nối `MaterialSlotService`: `Handle_ScalarPreview`/`Commit` (`SetSlotScalarParam`), `Handle_ColorPreview`/`Commit` (`SetSlotVectorParam`), `Handle_ResetParamsRequested` (`ClearSlotParams`+snapshot+RefreshParamPanel, thay STUB). Live-preview + undo/redo VALUE PASS. Dispatcher 2 row (`WBP_ParamScalarRow`/`WBP_ParamColorRow`) nâng 2-input `(ParamName,Value)` v1.1 — bug loại Q10 (contract producer không phủ consumer T4). Bug B3 (sót wire ParamName ở node preview màu → live-preview hỏng ngầm, bắt qua log `Param 'None'`) fix. **Lộ giới hạn kiến trúc:** undo mất slot-highlight vì Undo=destroy+respawn actor (`SelectedSlotIndex`/`Name` widget-state không sống qua respawn) → chốt **Hướng 1** (chấp nhận, đủ cho GATE); backlog `Bug-ParamUndo-SlotContextLost` + redesign undo Hướng 3 phiên sau (cuhoang chốt "đập xây lại"). 2 fix undo phụ (Equal-guard `OnMeshSelected` [K2] + `ApplyRestoredActor` v1.2 + seam #6). seam #7 (`EndParamSession`) tách T4b. Merge canonical: `WBP_FurnitureInventory.md` v3.32, 2 row v1.1, `DEVIATIONS.md`, `Open_Bugs.md`, `Learning_System.md` v1.6. Next: **phiên redesign undo (Hướng 3)** → T4b → T5. |
| 18–22/09/2026 (backfill tóm tắt) | ⚠ PROGRESS thiếu dòng cho T0 / U1 / U2.0–U2.3 (các phiên trước chỉ ghi Session_State + canonical). Tóm tắt: **T0** Automation Harness ĐÓNG (21/09) · **U1** Persistent Identity + Resolver ĐÓNG (21/09) · **U2.0–U2.2** backup+git, C++ command types + reader, struct V5 (22/09) · **U2.3** PARITY GATE PASS (24/09 — trước ghi nhầm 24/09). Chi tiết: `01_Session_State.md` "Last verified". |
| 24/09/2026 16:10 (**U2.4 + U2.5 ĐÓNG**) | **Interactive Edit Session + Commit — câu hỏi nhị phân U2 XANH:** kéo param → thả → Undo đảo đúng giá trị, KHÔNG respawn, slot/Inspector/gizmo còn; Redo áp lại đúng. BP_UndoManager v1.21 (session vars + Begin/Commit/Cancel + Undo/Redo guard + `OnHistoryChanged`), WBP_FurnitureInventory v3.33 (2 handler Begin, Commit rewire, 4 Cancel seam, seam #6 refresh, seed đổi nguồn), 2 row v1.2 (`OnEditBegin`+ParamName). PIE PASS: SESS-01/03/04/05/07, HIST-01, interleave U5, U6b (bác bỏ nghi vấn ghi mồ côi), W3 Color. **Đóng thêm:** B-gizmo (treo 15/06), Bug-ParamSeed-MIFallback (C++ reader đọc MI, git `c23b585`), Bug-ColorWheel-SetColorBeforeBuild (`InteriorColorPicker` v1.1 — plugin chưa git). Deviations D-5..D-13. Chưa soi K2 phiên này. Tiếp: **U2.6** History-UI. |
| 24/09/2026 17:40 (**U2 ĐÓNG HẲN**) | U2.6 History-UI: broadcast `OnHistoryChanged` +`AppendEntry` +nhánh Snapshot Undo/Redo; HISTUI-01/02/03 PASS; `JumpToHistoryIndex` ✓K2. U2.7: Print tạm dọn + smoke PASS; ✓K2 `CommitInteractiveEdit`/`BeginInteractiveEdit`; `Data_Structures`/`Testing` v1.2/W1–W7/DEVIATIONS plan-level; **§11 PASS 7/7**. `S7G7T4b` absorbed. `InteriorColorPicker` git local. Tiếp: task card **U3**. |
