...
**Nguồn:** `import_raw/Gate1_SprintD_Execution_Opus.md`
**Phiên bản:** 1.0 | **Ngày:** 11/06/2026 — Fable 5 | Lighting_Mnger UE5.5.4
**Đối tượng đọc:** model thực thi (Sonnet/Opus). Tuân thủ `Rules/AI_Implementation_Rules.md` (Q1-Q7, L1-L10, bảng node). Làm **TỪNG TASK MỘT**, mỗi task có test riêng, làm xong **báo cuhoang** rồi mới qua task sau.
...
> ⚠️ Ghi vào `Data/Python_Scripts.md` (script số 7) sau khi chạy OK, kèm kết quả Updated/Missing.
...
```
> ⚠️ Nếu `FilterMaterialItems` hiện có dùng cách đọc property khác → MIRROR cách đó cho nhất quán. `ExportText_InContainer` chậm hơn typed-cast một chút nhưng an toàn với mọi kiểu field.
...
MỚI: FilterFurnitureRows(DT_FurnitureCatalog, CurrentSearchText,
     ...
     → Call DisplayPage
```
> Recent/Favorite (ActiveSpecialCategory != ""): nhánh đặc biệt KHÔNG gọi filter C++ — build AllFilteredFurnitureRows trực tiếp từ list RowName của BP_FurnitureUserPrefsManager (đã RowName-based sẵn), rồi DisplayPage.
  ...
  == Furniture →
    ...
    ForLoop Start → End (i):
      ...
      Construct Object from Class(BP_FurnitureItemView, Outer=Self)
        ...
        → SET VieName/EngName/ThumbnailSoft/MeshSoft/MeshFolderPath/BoundingSize/Description/Link/Category
...
```
> ⚠️ Node tạo object runtime: `Construct Object from Class` — BP_MaterialItem đang được tạo bằng nó trong DisplayPage Material. Nếu Material đang dùng node khác → dùng đúng node đó.

### TEST D.T5 (BROWSE — vertical slice checkpoint)
```
1. Mở inventory → grid furniture hiện, thumbnail load lazy
...
### 6.3 Favorite (đơn giản hóa — RowName có sẵn)
```
...
### 6.4 DetailPopup
`OnCardInfoClicked` đổi param từ DA sang ItemView. `WBP_DetailPopup` đọc VieName/Description/Link/Thumbnail từ ItemView. BTN_BuyLink: Launch URL(ItemView.Link).

### 6.5 Drag-drop spawn (NHẠY NHẤT)
```
On Drag Detected:
...
On Drop:
  ...
  DAPath: GIỮ tương thích — build từ convention nếu code cũ cần
  RowName là định danh chính từ D.T8
```
> ⚠️ Mở node graph cũ, liệt kê MỌI chỗ đọc `FurnitureDA.*`, map từng cái sang `ItemView.*` — làm checklist trước khi sửa.

### 6.6 Replace mode
`EnterReplaceMode/ExitReplaceMode/RefreshCardReplaceMode` dùng Regenerate All Entries — không đổi. `OnMeshSelected` Trigger 3 (Replace navigate folder): GIỮ nguyên đường DAPath ở D.T6, sửa hẳn ở D.T8.
...
### 1.2 ⭐ Sửa box-select guard (BẮT BUỘC)
...
4. Search text + folder đang chọn GIỮ NGUYÊN sau đóng/mở
...
### 7.1 BuildFolderTree
Hiện đọc DA_ để xây Map. Đổi nguồn: cần MeshFolderPath per row → 200k rows duyệt trong Blueprint = CHẾT.
...
3. Search + category + Recent/Favorite — tất cả hoạt động
```
...
Chỉ còn 1 đường spawn sau G1.T2 = `SpawnFurnitureCopy`. Thêm input param `RowName : Name` (default None) → trong hàm SET lên actor.

### 8.3 Snapshot mang RowName
`S_FurniturePlacement` thêm `RowName : Name`. CaptureSnapshot Step 3 ghi từ actor. RestoreSnapshot truyền vào SpawnFurnitureCopy. Snapshot Version GIỮ 3 (field mới default None = vô hại với snapshot cũ).
...
3. Copy/Paste/Duplicate/Undo → RowName theo đồ
```
...
- Xóa `FurnitureDA` khỏi WBP_FurnitureCard nếu không còn ai đọc.
- Hàm C++ `FilterFurnitureItems` cũ: GIỮ — đánh dấu deprecated trong comment.

### 9.3 Docs (version + ngày + giờ + phút)
- `Widgets/WBP_FurnitureInventory.md` → v3.0: luồng row-based, DisplayPage 2 mode, single instance.
- `Data/Data_Structures.md`: S_FurnitureData +ThumbnailSoft; S_FurniturePlacement +RowName; BP_FurnitureActor +RowName; DA_FurnitureItem đánh dấu legacy.
- `Data/Python_Scripts.md`: script 7 + nhắc reimport CSV.
- `Rules/Performance.md`: gạch mục TECHNICAL DEBT AllFurnitureItems (đã xử lý), ghi kết quả đo.
- `Rules/AI_Implementation_Rules.md`: thêm node mới vào bảng (Construct Object from Class, Get Data Table Row Names, Clear List Items, Get Visibility).
- `00_Core/01_Session_State.md` + `00_Core/PROGRESS.md` + `00_Core/DEVIATIONS.md`.
...
2. **Spawn combo khi đang Edit Mode:** chưa định nghĩa. Đề xuất chốt: spawn combo → ExitEditModeFull trước.
3. **Toán rotate quanh pivot cho paste-combo-xoay:** nợ từ S1.T15.
4. **Async spawn N mesh + select:** cần counter "loaded N/N" rồi mới SelectActors.
5. Combo member list dùng **RowName** (đã có từ D.T8) — KHÔNG dùng MeshPath.

## Gate 2 — First Packaged Build (sau Sprint 5)
1. Build packaged lần đầu. Smoke test: spawn/select/move/group/undo/save/load.
2. **Verify P1:** `UniqueID = Get Display Name` trong CaptureSnapshot — hành vi khác giữa editor/packaged.
3. **Verify P4:** GPU VRAM crash (Streamline) có theo vào packaged không.
4. Debug Print String: gate bằng biến `bDebugMode`.
5. Phím tắt: rà phím trùng editor (F8 = eject PIE).

## Ngưỡng scale tiếp theo
