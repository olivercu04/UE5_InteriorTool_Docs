...
| **Current Task** | **U2 — History đa hình + Mutation Boundary + Param Command — ĐANG THỰC THI.** Hướng B (additive hybrid): command entry kèm full snapshot, undo command targeted qua Resolver (U1), KHÔNG viết lại snapshot core (hoãn trụ E sang SUBTRACT sau U3). 8 sub-gate U2.0→U2.7, PARITY GATE ở U2.3, câu hỏi nhị phân ở U2.5. Có mục History-UI readiness (scaffolding cửa sổ History kiểu Photoshop). **U2.0→U2.3 ĐÓNG (U2.3 PARITY PASS 25/09).** Đang ở **U2.4 — Interactive Edit Session** (việc của cuhoang trong Blueprint Editor). Task card: `Sprints/Sprint7/21-09-2026_U2_HistoryMutationBoundary_TaskCard.md` (file PLAN — KHÔNG sửa ngược, as-built đọc ở `BP_UndoManager.md` v1.20 + `Data/MaterialSlotService_Reference.md`). |
...
| **Next** | **U2.4 — Interactive Edit Session (cuhoang, Blueprint Editor):** build session vars `Sess_Active`/`Sess_Cmd`/`Sess_Phase` (§5.1) + `BeginInteractiveEdit` (§5.2, đọc Before) + `CommitInteractiveEdit` (§5.4 — thuộc U2.5) + `CancelInteractiveEdit` (§5.5, rollback Before) → rewire 5 handler `WBP_FurnitureInventory` (§5.10: Begin/Preview/Commit + Cancel seam + History-refresh seam) → **quay lại thêm nhánh `Branch(Sess_Active)→CancelInteractiveEdit()` ở đầu `UndoLastAction`/`RedoLastAction`** (U2.3 cố ý bỏ vì hàm Cancel chưa tồn tại). PIE: kéo slider chưa commit → Print Before đúng (SESS-01); đổi selection giữa drag → rollback về Before (SESS-05), history count KHÔNG tăng. Reviewed sớm ở U2.3: `ApplyParamCommand` (§5.6) đã đối chiếu §5.6 — đúng. Rồi U2.5 câu hỏi nhị phân (PIE) → U2.6 History-UI (§6 accessors đã build, chỉ còn PIE test HISTUI-01..03) → U2.7 doc as-built + §11 comprehension. Sau U2 → U3 (Context/ChangeSet, đóng `Bug-ParamUndo-SlotContextLost`) → quay lại Sprint 7 G8-G10. |
