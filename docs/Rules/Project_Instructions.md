...
**Nguồn:** `import_raw/Project_Instructions_Updated.md`
**Phiên bản:** 3.0 | **Cập nhật:** 28/05/2026

---

> Nội dung này dùng để paste vào ô **Project Instructions** của Claude Project.
> Phần (A) là Instructions chính, phần (B) là Memory/Context bổ sung.
...
1. **Đầu mỗi session: đọc `00_Core/01_Session_State.md` TRƯỚC TIÊN** (trạng thái + TODO + bug).
2. Khi cần node flow chi tiết → `Blueprints/Blueprint_Logic_NodeFlow.md`.
3. Khi làm tính năng MultiSelect/Group/Combo/Material v1.2 → đọc thư mục `Planning/` (xem mục dưới).
4. KHÔNG đọc tất cả file doc ngay từ đầu.

### Trạng thái hiện tại (cập nhật từ 28/05/2026)

**ĐÃ XONG:** Change Material v1.1, UX Phase 2.1 (Gizmo, Nudge, Copy/Paste, Recent/Favorite), Resize Window 8 hướng, Multi-Select (Sprint 1), Box+Context Menu (Sprint 2), Group (Sprint 3), Edit Mode + Nested Group (Sprint 4), Sprint 4 Bug Fix.

**TIẾP THEO:** Gate 1 (fix B1 bIsRestoring + hợp nhất spawn) → Sprint D (Data Layer v2) → Sprint 5 Combo → Sprint 7 Material v1.2 → Sprint 6 Polish → Gate 2.

### Kế hoạch docs/Planning/ (cho tính năng tiếp theo)

- **00_Master_Plan** — tổng quan, 6 quyết định kiến trúc đã chốt
- **01_Audit / 02_Target / 03_Inheritance** — kiến trúc hiện tại → đích, chiến lược kế thừa
- **04_Sprint_Details** — task chia sprint, làm theo đây
- **05_Data_Structures** — struct/enum/variable/signature chính xác
- **06_Risk / 07_Testing / 08_Performance** — rủi ro, test, tối ưu máy yếu
- **09_AI_Implementation_Rules** (= `Rules/AI_Implementation_Rules.md`) — ĐỌC ĐẦU TIÊN khi thực thi
- **10_Execution_Discipline** (= `Rules/Execution_Discipline.md`) — chống đi lạc, chống bỏ cuộc
- **`00_Core/DEVIATIONS.md` / `00_Core/PROGRESS.md`** — file sống, cập nhật khi code
...
- Lệch khỏi plan về logic/scope → ghi `00_Core/DEVIATIONS.md`
...
Ghi version + ngày + giờ + phút. Cập nhật `00_Core/01_Session_State.md` + `Blueprints/Blueprint_Logic_NodeFlow.md`. Tạo doc riêng nếu tính năng phức tạp. Thêm node mới vào bảng node trong `Rules/AI_Implementation_Rules.md`.

---

## (B) MEMORY / CONTEXT BỔ SUNG
...
- "FurniturePivot" — BP_PivotActor (KHÔNG save)
...
### Bug VRAM crash
Document tại `Bugs/Bug_GPU_VRAM_Crash.md`. Workaround: restart editor mỗi 2-3 PIE, hoặc Standalone Game (Alt+P). Mọi hard ref clear ở Event End Play/Destruct.
...
PageSize=48. Tabs Furniture/Material. Category: All, Furniture, Home_Decor, Bathroom, Appliances, Architectural, Small_Appliances, Recent, Favorite.
