...
**Phiên bản:** 2.24 | **Cập nhật:** 25/09/2026 00:45 — Q10 bước 2: điểm xuất phát = `Brain/Chỉ mục hàm & biến.md` (tự sinh từ Architecture_Map Phần 5), rồi mới search toàn project.
...
2. **Bản đồ Producer → State → Consumer** — **mở `Brain/Chỉ mục hàm & biến.md` TRƯỚC** (cột Ghi/Đọc = producer/consumer
   đã vẽ, kèm luồng + bước + ✓K2/doc; bước chi tiết ở `Brain/Canvas/5x` và Architecture_Map Phần 5) — rồi search TOÀN
   PROJECT theo TÊN STATE (không chỉ tên feature mới) vì chỉ mục chỉ phủ các luồng đã vẽ. Mỗi dòng gắn nhãn: `[K2]` verify qua export thật · `[DOC]` chỉ có trong canonical,
...
| 2.24 | 25/09/2026 00:45 | Q10 bước 2: thêm điểm xuất phát `Brain/Chỉ mục hàm & biến.md` (tự sinh bằng `Brain/_tools/build.py` từ Architecture_Map Phần 5) — để AI mở bản đồ chi tiết đúng lúc cần, không đọc đầu phiên. |
