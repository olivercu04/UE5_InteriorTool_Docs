...
**Phiên bản:** 1.6 | **Cập nhật:** 10/06/2026 — 20:34 ICT | Actor riêng — input hub + multi-select hub + box-select hub + context-menu hub + group hub

> **v1.6 (Sprint 3 Group + Regression-fix + Refactor dispatcher):** thêm hệ Group (`Groups` array, helpers, CreateGroup/UngroupActors). **HỢP NHẤT DISPATCHER:** xóa `OnMeshSelected` + `OnMeshDeselected`, `OnSelectionChanged` là dispatcher selection DUY NHẤT. Xóa biến chết `MeshToReplace` (single). Mouse Pressed Step 7 bỏ nhánh Ctrl (mọi click defer). Đọc kèm `Sprint3_Regression_DualDispatcher_Log.md`.
...
FoundIdx : Integer (local/class, default -1)   ← dùng trong UngroupActors; PHẢI SET -1 đầu hàm
```
...
### ExpandSelectionWithGroups(InActors) → Array<BP_FurnitureActor>  ⭐ keystone
```
ForEach InActors (Actor):
  Cast → GET GroupID
  Branch (GroupID == ""):
    True  → ADD Actor (đồ rời → chính nó)
    False → GetGroupChildren(GroupID) → ADD tất cả (đồ trong group → cả group)
Return (đã dedupe)
```
...
Make S_GroupData(NewGID, name, ParentGroupID="", bIsLocked=false) → ADD to Groups
...
### UngroupActors(InGroupID) — Ctrl+Shift+G
```
SET FoundIdx = -1                              ← reset class var đầu hàm (KHÔNG dựa default)
Branch (InGroupID == "") → True: Return
False:
  GetGroupChildren(InGroupID) → ForEach1 (child):
     Loop Body → Cast → SET child.GroupID = ""    ← CHỈ set, kết thúc loop body
  ForEach1 Completed →                            ← cụm find/remove/capture RA KHỎI loop body
     ForEach2 (Groups, Index) → Break → so GroupID == InGroupID → SET FoundIdx = Index
     ForEach2 Completed →
       Branch (FoundIdx >= 0) → True: REMOVE INDEX(Groups, FoundIdx)
       → SyncGroupsToContainer
       → CaptureSnapshot("Ungroup")              ← chạy 1 LẦN
       → SelectActors(SelectedActors)            ← re-fire info bar
       → Broadcast OnGroupDestroyed(InGroupID)
```
**⚠️ 2 bug đã trả giá:** (1) toàn bộ chuỗi find/remove/capture nếu nằm trong Loop Body của ForEach children → chạy N lần (spam snapshot). PHẢI ở Completed. (2) FoundIdx là class var → default -1 chỉ áp dụng lúc construct, phải SET -1 đầu hàm mỗi lần gọi (nếu không → warning "remove invalid index [0/0]").
...
### PruneEmptyGroups()
Quét Groups, group nào GetGroupChildren.Length==0 → loại khỏi Groups. Gọi trong DeleteSelected.
