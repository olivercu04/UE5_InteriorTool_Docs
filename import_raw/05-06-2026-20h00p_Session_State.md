...
**Giai đoạn:** Sprint 1 (Multi-select) — **15/15 COMPLETE. Multi-select Move + Rotate + Scale đã SHIP.**
**Đang làm trên:** PROJECT TỔNG (master)
**Engine:** UE5.5.4
**Tiếp theo:** Sprint 2 — Box Select + Context Menu (theo plan_v3/04_Sprint_Details.md)
...
## SPRINT 1 — MULTI-SELECT (03-05/06) — COMPLETE

**Xong (15):** T1 variables, T2 S_ClipboardEntry, T3 BP_PivotActor, T4 SpawnOrUpdatePivot, T5 helper functions, T6 OnSelectionChanged, T7 Mouse Left Pressed refactor (Ctrl toggle), T8 WBP_MeshControls multi, T10 multi-Nudge, T11 multi-Copy/Paste/Duplicate, T12 multi-Snapshot, T14 Selection Info Bar + OnSceneChanged dispatcher, **T15 Multi-Rotate/Scale qua Pivot (Transform Composition)**.
...
- Transform Composition: `Make Relative Transform` + `Compose Transforms` cho Rotate + Scale
...
| **DuplicateMesh infinite loop + spawn N×N đồ** | Phần spawn nối nhầm Loop Body của ForEach MaxRightEdge → chuyển sang COMPLETED |
| **Chuyển mode (Move/Rotate/Scale) khi multi-select → gizmo nhảy về đồ đơn** | WBP_MeshControls BTN_Move/Rotate/Scale: kiểm tra `SelectedActors.Length >= 2` → dùng `GizmoPivotActor`, else `SelectedFurnitureActor` |
...
Code chạy **1 lần** phải nối vào **Completed** của ForEach, KHÔNG nối Loop Body. Bug DuplicateMesh (infinite loop + spawn thừa N×N) do phần spawn nằm trong Loop Body của ForEach MaxRightEdge.
→ **Khi gặp infinite loop / số lượng nhân lên: kiểm tra NESTING (Loop Body vs Completed) TRƯỚC khi nghi aliasing.**

### ⭐ Transform Composition — T15 (05/06)
Multi-rotate/scale qua Pivot dùng 2 node: `Make Relative Transform(childWorld, pivotWorld)` lúc drag-start → `Compose Transforms(relT, currentPivot)` mỗi frame. KHÔNG tính delta tay. Gọi RefreshOffsets tại drag-start (OnMousePressed Cast → PivotActor), KHÔNG mỗi frame (mất điểm tham chiếu → đồ đứng yên).

### Multi-select gizmo lifecycle
Đổi target gizmo: luôn DeactivateGizmo TRƯỚC ActivateGizmo. Length 0/1/>=2 xử lý riêng trong UpdateGizmo. Khi chuyển mode: kiểm tra Length >= 2 để chọn đúng target (GizmoPivotActor hay SelectedFurnitureActor).

### Array pass-by-reference
Function param Array thường pass-by-reference. Build array bằng CLEAR + ForEach ADD element-by-element nếu cần độc lập.

### Teleport = True trong Set Actor Transform
Set transform mỗi frame (60fps) → dùng Teleport=True. Physics engine không can thiệp → mượt. Teleport=False mỗi frame → physics sweep → giật.
