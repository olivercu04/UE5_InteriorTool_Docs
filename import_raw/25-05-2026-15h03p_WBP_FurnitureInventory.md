...
**Phiên bản:** 2.2 | **Cập nhật:** 25/05/2026 — 15:03 ICT
...
#### OnMeshSelected(SelectedActor) — v1.1 + v1.3
```
← v1.3 Trigger 3: nếu đang Replace mode → update MeshToReplace + navigate folder
Branch bIsReplaceMode == True:
  T →
    Get All Actors Of Class(BP_FurnitureInputManager) → GET [0] → Cast → FurnitureInputRef
    SET MeshToReplace = SelectedActor (FurnitureInputRef)

    Cast SelectedActor → BP_FurnitureActor → GET DAPath
    Load Asset Blocking(DAPath) → Cast DA_FurnitureItem → GET MeshFolderPath
    Branch MeshFolderPath != "":
      T → FilterByFolderPathWithUI(MeshFolderPath)
      F → (không làm gì)

    ← merge → tiếp tục logic bên dưới
  F → (tiếp tục)

← v1.1: Material mode update
Branch CurrentInventoryMode == Material:
...
#### OnMeshDeselected — v1.1 + v1.3
```
← v1.3 Trigger 1: thoát Replace mode khi deselect
Branch bIsReplaceMode == True:
  T → SET bIsReplaceMode = False (self)
      Get All Actors Of Class(BP_FurnitureInputManager) → GET [0] → Cast
      → SET bIsReplaceMode = False
      → SET MeshToReplace = None
      Call ExitReplaceMode   ← ẩn BTN_ChangeMesh trên tất cả cards
  F → (tiếp tục)

← v1.1: Material mode
Branch CurrentInventoryMode == Material:
  T → Collapsed HB_SlotSwatches
      SET TargetFurnitureActor = None, SET SelectedSlotIndex = -1
```

#### EnterReplaceMode (Function) — v1.3
```
