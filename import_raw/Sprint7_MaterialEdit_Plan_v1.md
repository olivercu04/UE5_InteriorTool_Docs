...
**Version:** 1.0 | **Cập nhật:** 03/07/2026
...
TEST G1 (chuỗi debug, chưa UI): apply MI theo tên vào 1 actor → đúng slot; SetScalar param có thật → đổi trên mesh; param bịa → trả **false**; slot nguyên bản chưa MID → SetVector vẫn ăn (MID-on-demand) + record có PathFallback; Serialize → Print JSON đọc được; Parse ngược → so khớp.
...
5. Reset Slot/Reset All đổi sang: mức 2 = apply lại material gốc qua service (PathFallback/gốc), mức 1 = `ClearSlotParams` (nút mới chưa cần UI — G5).
...
1. **RestoreMyMaterialSlots** (Custom Event trên BP_FurnitureActor — MỘT đường duy nhất):
```
▶→ Branch(MaterialSlots.Length == 0)
     True  ▶→ Branch(MaterialOverrides cũ có dữ liệu)          ← đường LEGACY
              True ▶→ BuildRecordsFromLegacy(...) ●→ SET MaterialSlots
              False → [xong — mesh nguyên bản]
     (merge)
▶→ ForEach MaterialSlots (record):                              ← async load từng MI (R1)
     RowName != "" → tra DT lấy path; rỗng → PathFallback
     Async Load → Completed → ApplyLoadedMaterialToSlot + ApplyParamsJsonToSlot
```
...
`Session_State` + `PROGRESS` (S7 DONE); `DEVIATIONS` — 3 dòng soạn sẵn: (1) *Replace mesh đổi hành vi: giữ material khi slot trùng tên (P5) — v1.1 là reset*; (2) *Save format v2 MaterialSlots additive, fields cũ thành legacy read-only*; (3) *Q1/Q2 chốt theo số liệu G0: ...*; tạo `MaterialSlotService_Reference.md` + `MaterialSurvey_Reference.md`; bump `ChangeMaterial.md` → v2.0, `WBP_FurnitureInventory.md`, `BP_FurnitureActor.md`, snapshot/EMS docs, `Combo` schema doc; node reference thêm node mới phát sinh (cuhoang xác nhận từng cái).
